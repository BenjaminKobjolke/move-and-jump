...
// Mirrors ShortcutUtils' own regex; used by mapKey() for the `commands` column,
// where F1-F12 are a legitimate binding.
const FUNCTION_KEY = /^(F[1-9]|F1[0-2])$/;

// What a single-key binding may be. Deliberately narrower than the set of keys
// Thunderbird can spell (Comma, Space, Up, F5, ...): the experiment's keydown
// listener matches a binding by uppercasing `event.key` directly, so anything
// whose Thunderbird name is not just its uppercased character — "Comma" for ",",
// "F5" for a two-character key — would be recorded, displayed, and then never
// fire. Widening this means teaching experiments/keys/implementation.js the same
// aliases first.
const SINGLE_KEY = /^[A-Z0-9]$/;
...
/**
 * Check a shortcut is usable as a single-key binding: a letter or a digit,
 * optionally with Shift. See SINGLE_KEY for why nothing else is allowed.
...
export function validateSingleKey(shortcut) {
  ...
  if (!SINGLE_KEY.test(key)) return "invalid-key";
  return null;
}
