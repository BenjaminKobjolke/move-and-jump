export const DEFAULT_OPTIONS = {
  ...
  resizeToFit: true,
  // Extra fields the /filter command searches. Sender and subject are always
  // searched; these two widen it (see docs/FILTER_EMAILS.md).
  filterBody: false,
  ...
  centerOnParent: true,
  // Close (destroy) the search window on dismiss instead of minimizing it, so
  // the next trigger creates a fresh one. Costs a little open latency; leaves no
  // taskbar entry behind between uses.
  recreateWindow: false,
  // Single-key bindings, command name → shortcut ("S", "Shift+S"). These are
  // the ones Thunderbird's commands API refuses to take; experiments/keys
  // installs them instead. A command can have both this and a normal shortcut.
  singleKeys: {},
};
